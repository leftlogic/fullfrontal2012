...
function Track(context) {
  ...
  function getXY(event) {
    event = event.pageX !== undefined ? event : event.touches.length ? event.touches[0] : { pageX: 0, pageY: 0 };
    // console.log('y', event.pageY, document.body.scrollTop, contextPos.y);
    return {
      x: event.pageX - contextPos.x, // document.body.scrollLeft - 
      y: event.pageY - contextPos.y // document.body.scrollTop - 
    };
  }
